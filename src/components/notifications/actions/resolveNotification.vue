<template>
  <div id="resolve-notification" class="notification-action-popup__box notification-action-popup">
    <div class="popup__box-header">
      <h4 class="popup__box-title">Resolve alert</h4>
    ...
    <div class="popup__box-main" style="height: unset">
      ...
      <div class="popup__box-buttons">
        ...
        <button class="button__large button__accent handle" @click="submit">Resolve</button>
      </div>
...
export default {
  ...
  methods: {
    close () {
      this.$store.commit(HIDE_RESOLVE_DIALOG)
    },
    submit () {
      const body = {
        comment: this.comment
      }
      this.$store.dispatch(RESOLVE_NOTIFICATION, body)
    }
  }
}
</script>
