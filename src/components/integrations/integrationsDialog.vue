<template>
  <div class="main-client-dialog" style="width: 840px;">
    ...
    <div class="popup__box-header">
      <h4 class="popup__box-title">Integrations</h4>
    ...
    <div class="integrations-page__content">
      ...
      <div v-else class="tabs" style="height: 100%">
        ...
        <div class="tabs__content" style="height: 100%;">
          <configured
            :configured-integrations="configuredIntegrations"
            :environments="environments"
            :preChosenIntegrationId="chosenIntegrationId"
            @update-integrations-list="onLoad"
...
<script>
import { HIDE_INTEGRATIONS } from '@/store/actions/localActions'
import { SCENARIOS_REQUEST, AVAILABLE_INTEGRATIONS_REQUEST } from '@/store/actions/config'
import store from '@/store'
import createNew from '@/components/integrations/createNew'
import configured from '@/components/integrations/configured'
import requestNew from '@/components/integrations/requestNew'
...
export default {
  ...
  methods: {
    ...
    onLoad (id) {
      console.log('integrationsDialog. onLoad')
      this.chosenIntegrationId = id
      this.loading = true
      this.$store.dispatch(SCENARIOS_REQUEST)
      this.$store.dispatch(AVAILABLE_INTEGRATIONS_REQUEST)
      const requestObj = {
        service: 'integrations',
        path: '/integrations/configured/all',
        method: 'GET',
        options: {}
      }
      const msg = 'GET integrations'
      const popupOptions = {
        showDialog: true
      }
      this.$store.dispatch(REMOTE_REQUEST, { requestObj, msg, popupOptions })
        ...
        .catch((error) => {
          const errorDescription = {
            ...
            eventType: 'error',
            showError: true,
            message: 'Can\'t load integrations.'
          }
          this.$store.dispatch(TRIGGER_ERROR, errorDescription)
          console.error('integrationsDialog. onLoad. Error:', error)
          this.loading = false
          // this.$router.go(-1)
        })
    },
    addIntegration (newIntegration) {
      console.log('integrationsDialog. methods. addIntegration. newIntegration:', newIntegration)
      this.configuredIntegrations.push(newIntegration)
    },
    deleteIntegration (integrationId) {
      console.log('integrationsDialog. methods. deleteIntegration. IntegrationId:', integrationId)
      const elementIndex = this.configuredIntegrations.findIndex(
        integrationObj => integrationObj.integration_id === integrationId
      )
      if (elementIndex !== -1) {
        console.log('integrationsDialog. methods. deleteIntegration. elementIndex:', elementIndex)

        this.configuredIntegrations.splice(elementIndex, 1)
      } else {
        console.log('integrationsDialog. methods. deleteIntegration. elementIndex wasn\'t found')
      }
    }
  }
}
